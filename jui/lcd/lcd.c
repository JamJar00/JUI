...
void fill_rectangle(rectangle_t r, uint16_t col) {
    write_cmd(COLUMN_ADDRESS_SET);
    write_data16(r.left);
...
void fill_rectangle_indexed(rectangle_t r, uint16_t* col) {
    init_pix_stream(r);
    pix_stream(col, (r.right - r.left) * (r.bottom - r.top));
}

void init_pix_stream(rectangle_t r) {
    write_cmd(COLUMN_ADDRESS_SET);
    write_data16(r.left);
    write_data16(r.right);
    write_cmd(PAGE_ADDRESS_SET);
...
void pix_stream(uint16_t* col, uint16_t size) {
    while(size-- > 0)
        write_data16(*col++);
}
...
void clear_screen() {
    display.x = 0;
    display.y = 0;
    rectangle_t r = {0, display.width-1, 0, display.height-1};
