...
int main() {
  ...
  int i;
  component_t image_components[COUNT];
  component_t button_components[COUNT];
  for (i = 0; i < COUNT; i++) {
    image_create(image_components + i, NO_STYLE, "smile.bmp");
    button_create(button_components + i, ICONMENU_BUTTON_STYLE, image_components + i, icon_button_callback);
  }
